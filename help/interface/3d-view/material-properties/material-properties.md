...
breadcrumb-title: ''
description: Configurez les propriétés du matériau dans la vue 3D pour prévisualiser et ajuster l’apparence de vos matériaux de Substance sur les objets 3D.
...
La [vue 3D](../../../interface/3d-view/3d-view.md) restitue la surface des modèles à l&#39;aide d&#39;un programme appelé *shader*. Le shader définit le matériau
appliqué au modèle à l&#39;aide d&#39;une liste de propriétés qui ont un impact sur divers aspects de l&#39;apparence du modèle.

Le menu **Matériaux** de la vue 3D vous permet de vérifier quel shader est utilisé pour chacun des matériaux de la scène.
...
Les [modèles de graphe](../../../compositing-graphs/creating-compositing-gra/creating-a-substance-compositing-graph.md#graph-templates) par défaut et les [échantillons de matériau](../../../compositing-graphs/creating-compositing-gra/creating-a-substance-compositing-graph.md#material-samples) inclus dans Designer sont tous basés sur le modèle OpenPBR.

Les propriétés de ce shader suivent la [référence de paramètre d&#39;OpenPBR](https://academysoftwarefoundation.github.io/OpenPBR/#parameterreference) et sont *partagées* sur le pixelliseur,
...
|---------------------------------|---------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Répétition | Flottant | 1.0 | Le nombre de répétitions de textures dans une cellule d&#39;UV, où une valeur plus élevée<br/>entraîne plus de répétitions de textures. |
| Activ. taille physique à partir du graphe | Booléen | False | Ajustez automatiquement la répétition en fonction de la [Taille physique](../../../compositing-graphs/graph-parameters/graph-parameters.md)<br/>du graphe afin de représenter le matériau à l&#39;échelle appropriée. |
| Échelle UV | Flottant2 | 1.0, 1.0 | Règle l&#39;échelle de la répétition selon un facteur distinct pour U et V, où<br/>une valeur plus élevée entraîne davantage de répétitions de texture. |
...
| Couleur | Flottant 3 (RGB) | 0.8, 0.8, 0.8 | Couleur de la réflexion provenant de la base diffuse et métallique. |
...
| Couleur | Flottant 3 (RGB) | 1.0, 1.0, 1.0 | Couleur du reflet du specular. (Contrôle la teinte de contour physique pour les métaux,<br/> et une teinte globale non physique pour les diélectriques) |
| Rugosité | Flottant | 0.3 | La rugosité de la réflexion du specular. Les valeurs faibles produisent des <br/>reflets plus nets, tandis que les valeurs élevées produisent des reflets plus flous. |
| Anisotropie | Flottant | 0.0 | La polarisation directionnelle de la rugosité de la base métallique/diélectrique, qui se traduit<br/>par des reflets de plus en plus étirés le long de la direction de la tangente. |
...
| Couleur | Flottant 3 (RGB) | 1.0, 1.0, 1.0 | Contrôle la couleur de la base transparente en raison de l&#39;absorption volumétrique<br/>de la loi de Beer sous la surface. |
...
| Graphique de dispersion | Flottant 3 (RGB) | 0.0, 0.0, 0.0 | Contrôle la couleur de la lumière diffusée volumétriquement à l’intérieur de la base transparente. |
...
| Paramètre | Type | Par défaut | Description |
|--------------|--------------|----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Poids | Flottant | 0.0 | Poids du mélange qui compose la base diélectrique opaque entre <br/>réflexion diffuse et subsurface scattering. |
| Couleur | Flottant 3 (RGB) | 0.8, 0.8, 0.8 | Couleur de réflexion observée pour un milieu avec subsurface scattering. |
| Rayon | Flottant | 1.0 | Échelle de longueur du chemin libre moyen pour le subsurface scattering. |
| Échelle de rayon | Flottant 3 (RGB) | 1.0, 0.5, 0.25 | Multiplicateur RGB à subsurface_radius, donnant la diffusion par canal<br/>tracés sans moyenne. |
| Anisotropie | Flottant | 0.0 | Contrôle la fonction de phase de la subsurface scattering, où zéro<br/>dispersion la lumière uniformément, les valeurs positives la dispersion vers l&#39;avant et les valeurs négatives<br/>la dispersion vers l&#39;arrière. |
...
| Paramètre | Type | Par défaut | Description |
|------------|--------------|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| Poids | Flottant | 0.0 | Le poids de la présence d&#39;une couche réfléchissante de revêtement transparent sur le dessus du matériau.<br/>À utiliser pour des matériaux tels que la peinture de voiture ou un calque huileux. |
| Couleur | Flottant 3 (RGB) | 1.0, 1.0, 1.0 | Couleur de la transparence de la couche de vernis, due à l’absorption dans le revêtement. |
| Rugosité | Flottant | 0.0 | La rugosité des reflets du pelage transparent.<br/>Plus la valeur est faible, plus le reflet est net. |
| Anisotropie | Flottant | 0.0 | Biais directionnel de la rugosité du calque du pelage transparent<br/>entraînant des tons clairs de plus en plus étirés dans la direction de la tangente du pelage. |
| IOR | Flottant | 1.6 | Indice de réfraction de la couche de vernis. |
...
| Couleur | Flottant 3 (RGB) | 1.0, 1.0, 1.0 | Couleur de la couche de fibres. |
...
| Couleur | Flottant 3 (RGB) | 1.0, 0.0, 0.0 | Couleur de la lumière émise. |
...
| Normale | Flottant 3 (RGB) | 0.5, 0.5, 1.0 | Normale géométrique d’entrée pour la surface. |
| Tangente | Flottant 3 (RGB) | 1.0, 0.5, 0.0 | Tangente géométrique d’entrée. |
| Normale du revêtement | Flottant 3 (RGB) | 0.5, 0.5, 1.0 | Normale d’entrée pour la couche de revêtement. |
| Tangente du revêtement | Flottant 3 (RGB) | 1.0, 0.5, 0.0 | Tangente géométrique d’entrée pour la couche de revêtement. |
...
| Échelle de height | Flottant | 1.0 | Échelle du displacement ou de la bosse en unités d’espace de scène.<br/>L&#39;amplitude et la direction de l&#39;échelle sont indépendantes de la valeur du niveau d&#39;height. |
| Occlusion ambiante | Flottant | 1.0 | Carte d&#39;Ambient occlusion pour obscurcir les zones occultées.<br/>Blanc (1.0) signifie entièrement éclairé, noir (0.0) signifie entièrement occulté. |

+++

### Compatibilité avec les graphes existants

Certaines propriétés d’OpenPBR ont des identifiants d’utilisation différents de ceux des autres modèles inclus dans Designer.
...
Le modèle Adobe Standard Material (ASM) a été introduit dans Designer 11.2 et a été mis à jour avec le shader par défaut de Designer
...
Le but du modèle UsdPreviewSurface est de prévisualiser les matériaux avec un ensemble de fonctionnalités de base favorisant la compatibilité
