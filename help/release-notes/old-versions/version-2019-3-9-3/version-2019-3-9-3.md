---
helpx_url: "https://helpx.adobe.com/fr/substance-3d-designer/release-notes/old-versions/version-2019-3-9-3.html"
breadcrumb-title: ''
...
* **La gestion des couleurs est appliquée de 3 manières différentes**\
  ...
  * **Entrées et sorties graphiques et exportation bitmap**\
    Lors de la lecture et de l’écriture d’images bitmap à l’entrée et à la sortie d’un graphique, la gestion des couleurs peut ajuster l’image et la convertir pour qu’elle fonctionne dans les bonnes conditions. Cela peut être contrôlé dans la préférence principale pour les bitmaps dans le graphique et modifié dans la boîte de dialogue d’exportation pendant le processus d’exportation. L’enregistrement d’une image à partir de la vue 2D offre également un moyen de choisir l’espace colorimétrique.

    ![](../../../assets/color-export-outputs.png){width="400px"}

    ![](../../../assets/color-export-2d.png){width="505px"}

### Nouvelle courbure de Mesh Baker

![](../../../assets/banner-curvature-1.png)

La courbure de mesh baker a été refaite à partir de zéro. Il tire désormais parti du lancer de rayons, qui permet l’accélération GPU, mais ajoute également la prise en charge des intersections de filet.

Pour plus d&#39;informations, consultez la documentation [Courbure à partir du maillage](https://experienceleague.adobe.com/en/docs/substance-3d/bakers/bakers-settings/curvature-from-mesh).

>[!NOTE]
>
> L’ancienne courbure du boulanger de filet est désormais obsolète. Nous vous recommandons de passer à la nouvelle version.

### Amélioration de l’Occlusion ambiante grâce à Mesh Baker

![](../../../assets/banner-ao-v2.png)

Le paramètre Occlusion ambiante à partir de Mesh baker dispose désormais d’un nouveau paramètre appelé « Plan au sol » qui simule un plan sous le filet pour produire une occlusion au sol. Cela peut aider à produire des résultats d’aspect plus intéressants pour la géométrie statique ou les objets non organiques. Le plan au sol se trouve par défaut au bas du cadre de sélection du maillage, mais peut être poussé davantage avec des paramètres dédiés.

Pour plus d&#39;informations, consultez l&#39;[Occlusion ambiante à partir de la documentation Mesh](https://experienceleague.adobe.com/en/docs/substance-3d/bakers/bakers-settings/ambient-occlusion-from-mesh).
...
* **Mode d&#39;aperçu plus rapide**\
  Le passage en mode Aperçu lors de la modification des paramètres de graphique est désormais beaucoup plus rapide, en particulier sur les graphiques comportant de nombreux paramètres exposés. Le mode Aperçu est désormais un onglet dédié qui facilite le va-et-vient entre l&#39;éditeur de paramètres et son aperçu.
...
* **Visible Si prend désormais en compte les gadgets de transformation 2D**\
  Lorsqu&#39;un widget 2D de transformation possède une condition If visible, il peut désormais être masqué dans la vue 2D, ce qui le rend contextuel par rapport aux paramètres.

### Nouveau contenu

![](../../../assets/banner-content-8.jpg)

Cette version présente le nouveau nœud **Atlas scatter**. Ce nœud permet de lire une image d&#39;atlas (une image contenant des sous-images) et de séparer automatiquement son contenu en une composante individuelle qui peut ensuite être diffusée aléatoirement. Ce nœud peut être utilisé par example pour placer des branches et des feuilles sur un matériau de sol dirt. La [Substance Source](https://source.substance3d.com/allassets?q=atlas) inclut désormais de nombreux matériaux Atlas qui peuvent être dispersés avec ce nouveau nœud.
