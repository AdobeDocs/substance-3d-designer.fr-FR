...
| --- | --- |
| **Méthode** | Contrôle la façon dont les textures cuites seront stockées avec le package de Substance.Valeurs possibles :<ul data-preserve-html="true"><li data-preserve-html="true"><strong>Incorporé</strong> : la texture cuite est stockée dans un sous-dossier en regard du package de Substance avec un nom spécifique.</li><li data-preserve-html="true"><strong>Lié</strong> (par défaut) : les textures bakées sont stockées dans le dossier défini, puis référencées dans le package de Substances.</li></ul> |
...
Par défaut, chaque baker hérite des valeurs par défaut (voir ci-dessus). La taille (résolution) peut par exemple être remplacée en cliquant sur la cellule sur la ligne du boulanger. Cela est vrai pour les autres paramètres de la ligne.
